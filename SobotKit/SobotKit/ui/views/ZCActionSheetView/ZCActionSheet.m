...
- (instancetype)initWithDelegate:(id<ZCActionSheetDelegate>)delegate selectedColor:(UIColor *)color CancelTitle:(NSString *)cancelTitle OtherTitles:(NSString *)otherTitles, ...{
    ...
    _selectColor = color;
    if(_selectColor == nil){
        _selectColor = UIColorFromKitModeColor(SobotColorTextLink);
    }
    ...
    [sobotGetCurWindow() addSubview:self];
    
    // sheet
    UIView *sheetView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, ScreenWidth, 0)];
    sheetView.backgroundColor = [ZCUIKitTools zcgetLightGrayBackgroundColor];
    ...
    [btn setBackgroundImage:[self createImageWithColor:UIColorFromKitModeColor(SobotColorBgMainDark2)] forState:UIControlStateNormal];
    [btn setBackgroundImage:[self createImageWithColor:[ZCUIKitTools zcgetLightGrayBackgroundColor]] forState:UIControlStateHighlighted];
    [btn setTitle:cancelTitle forState:UIControlStateNormal];
    [btn setTitleColor:[ZCUIKitTools zcgetChatTextViewColor] forState:UIControlStateNormal];
    btn.autoresizingMask = UIViewAutoresizingFlexibleWidth|UIViewAutoresizingFlexibleTopMargin;
    btn.titleLabel.font = SobotFontSTHeitiSC17(17);
    btn.tag = 0;
...
-(instancetype)initWithDelegate:(id<ZCActionSheetDelegate>)delegate selectedColor:(UIColor *)color showTitle:(NSString *)title CancelTitle:(NSString *)cancelTitle OtherTitles:(NSString *)otherTitles, ...{
    ...
    [titleLabel setFont:SobotFont14];
...
- (void)setupBtnWithTitle:(NSString *)title{
    ...
    if(_selectIndex==_btnTag){
        ...
        [btn setTitleColor:UIColorFromKitModeColor(SobotColorTextMain) forState:UIControlStateNormal];
    }
    btn.titleLabel.font = SobotFontSTHeitiSC17(17);
    btn.tag = _btnTag;
    [btn addTarget:self action:@selector(sheetBtnClick:) forControlEvents:UIControlEventTouchUpInside];
    ...
    line.backgroundColor = [ZCUIKitTools zcgetCommentButtonLineColor];
    [btn addSubview:line];
    _btnTag ++;
}
