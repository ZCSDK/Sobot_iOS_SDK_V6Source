...
@interface ZCLanguageView ()<UITableViewDelegate,UITableViewDataSource>{
...
-(void)updataPage{
    self.titleLabel.text = SobotKitLocalString(@"选择语言");
    if ([[[ZCUICore getUICore] getLibConfig].language isEqual:@"العربية"] || ([ZCLibClient getZCLibClient].libInitInfo.absolute_language && [sobotConvertToString([ZCLibClient getZCLibClient].libInitInfo.absolute_language) isEqualToString:@"ar"])) {
        self.titleLabel.text = @"اختر اللغة";
    }
...
//    [headerView setBackgroundColor:UIColorFromKitModeColor(SobotColorBgSub2Dark1)];
    ...
    [searchIcon setFrame:CGRectMake(13, 10, 16.07, 16.07)];
...
-(void)searchTextChanged:(UITextField *) field{
    ...
    if(sobotConvertToString(text).length > 0){
        ...
        for (ZCLanguageModel *model in self.searchArray) {
            // 不区分大小写
            NSString *lowerName = [sobotConvertToString(model.name) lowercaseString];
            NSString *lowerText = [sobotConvertToString(text) lowercaseString];
            if ([lowerName containsString:sobotConvertToString(lowerText)]) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (sobotConvertToString(_searchField.text).length >0) {
        textLabel.attributedText = [ZCUIKitTools getOtherColorString:self.searchField.text Color:[ZCUIKitTools zcgetServerConfigBtnBgColor] withString:model.name];
    }else{
...
-(void)createTitleView{
    ...
    self.titleLabel = [[UILabel alloc] initWithFrame:CGRectMake(16,0, _topView.frame.size.width- 16*2, ZCSheetTitleH)];
...
//    [self.topView addSubview:self.backButton];
    ...
    bottomLine.backgroundColor = UIColorFromKitModeColor(SobotColorBgTopLine);// [ZCUIKitTools zcgetCommentButtonLineColor];
