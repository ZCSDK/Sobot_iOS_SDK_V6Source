...
- (instancetype)initWithFrame:(CGRect)frame withShowInView:(UIView *)bgView url:(NSURL *)url Image:(UIImage * _Nullable)image{
...
-(void)showControlsView{
    ...
    [_btnBack setImage:[SobotUITools getSysImageByName:@"icon_video_close"] forState:0];
    ...
    [_btnPlay setImage:[SobotUITools getSysImageByName:@"zcicon_video_fullplay"] forState:0];
    ...
    _menuView = [[UIView alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(self.frame) - 52 - (sobotIsIPhoneX()?34:0), CGRectGetMaxX(self.frame), 52)];
    ...
    [_btnPause setImage:[SobotUITools getSysImageByName:@"icon_video_menu_pause"] forState:0];
    
    _labStartTime = [[UILabel alloc] initWithFrame:CGRectMake(62, 5, 35, CGRectGetHeight(_menuView.frame) - 10)];
    [_labStartTime setFont:SobotFont11];
    [_labStartTime setTextColor:UIColorFromKitModeColor(SobotColorTextWhite)];
    ...
    [_labEndTime setFont:SobotFont11];
    [_labEndTime setTextColor:UIColorFromKitModeColor(SobotColorTextWhite)];
    ...
    _sliderProgress.tintColor = UIColorFromKitModeColor(SobotColorTextWhite);
    _sliderProgress.minimumTrackTintColor = UIColorFromKitModeColor(SobotColorTextWhite); // 已走过
    _sliderProgress.maximumTrackTintColor = UIColorFromKitModeColor(SobotColorTextSub); // 未走过
    _sliderProgress.thumbTintColor = UIColorFromKitModeColor(SobotColorTextWhite);        // 滑块颜色
    ...
    UIImage *img = [SobotImageTools sobotImageWithColor:UIColorFromKitModeColor(SobotColorTextWhite)];
    img = [SobotImageTools sobotScaleToSize:CGSizeMake(12, 12) with:img];
...
-(UIView *) getTipsView{
    if(!_tipsView){
        ...
        [lab1 setTextColor:[ZCUIKitTools zcgetServiceNameTextColor]];
        [lab1 setFont:SobotFont17];
        [lab1 setText:SobotKitLocalString(@"播放失败")];
        [_tipsView addSubview:lab1];
        
        UILabel *lab2 = [[UILabel alloc] initWithFrame:CGRectMake(0, 40, 220, 24)];
        [lab2 setTextColor:[ZCUIKitTools zcgetTimeTextColor]];
        [lab2 setFont:SobotFont14];
        [lab2 setText:SobotKitLocalString(@"视频文件损坏，无法播放")];
        [_tipsView addSubview:lab2];
        
        
        UIButton *btn = [UIButton buttonWithType:UIButtonTypeCustom];
        [btn setTitle:SobotKitLocalString(@"确定") forState:0];
        [btn addTarget:self action:@selector(goBack:) forControlEvents:UIControlEventTouchUpInside];
        [btn setFrame:CGRectMake(220-60, 110-38, 50, 28)];
        [btn setBackgroundColor:[ZCUIKitTools zcgetBgBannerColor]];
...
-(void)pause:(UIButton *) btn{
    if(self.player){
        if (self.player.rate == 0) {
            ...
            if(_btnPlay){
                _btnPlay.hidden = NO;
                [_btnPause setImage:[SobotUITools getSysImageByName:@"icon_video_menu_play"] forState:0];
            }
        }
    }
}
