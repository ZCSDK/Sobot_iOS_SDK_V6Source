...
//        [ZCLibClient getZCLibClient].libInitInfo.platform_key = @"";
    ...
    if([@"3.4.1" isEqual:code]){
        ...
        [ZCSobotApi openZCChat:[ZCGuideData getZCGuideData].kitInfo with:sender pageBlock:^(id  _Nonnull object, ZCPageStateType type) {
    ...
    if([@"3.4.2" isEqual:code]){
        ...
        [ZCSobotApi openZCChatListView:[ZCGuideData getZCGuideData].kitInfo with:self onItemClick:^(SobotClientBaseController * _Nonnull object, ZCPlatformInfo * _Nonnull info) {
            [ZCLibClient getZCLibClient].libInitInfo.app_key = info.app_key;
            [ZCSobotApi openZCChat:[ZCGuideData getZCGuideData].kitInfo with:self pageBlock:^(id  _Nonnull object, ZCPageStateType type) {
    ...
    if([@"3.4.3" isEqual:code]){
        ...
        [ZCSobotApi openZCServiceCenter:[ZCGuideData getZCGuideData].kitInfo with:self onItemClick:^(SobotClientBaseController * _Nonnull object) {
            [ZCSobotApi openZCChat:[ZCGuideData getZCGuideData].kitInfo with:self pageBlock:^(id  _Nonnull object, ZCPageStateType type) {
